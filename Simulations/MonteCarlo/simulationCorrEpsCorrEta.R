...
# Function to produce random SPD matrix (taken from R user list)
...
# Preference shocks: distributed mv normal
D    = J*L*K
MU1  = rep.int(0,D)
SIG1 <- posdef(n=D)
epsB <- mvrnorm(N,MU1,SIG1)
epsB <- array(c(epsB), dim=c(N,J,L,K))
...
# Earnings shocks: distributed mv normal
eta <- array(rnorm(N*L*K), dim=c(N,L,K))
D    = L*K
MU1  = rep.int(0,D)
SIG1 <- posdef(n=D)
eta <- mvrnorm(N,MU1,SIG1)
eta <- array(c(eta), dim=c(N,L,K))
...
dat <- cbind(inder,inderRand,obsY,obsYrand,X)
colnames(dat)[1:4] <- c("choice","choiceRand","wage","wageRand")
...
save(varY,file="varYCorrEpsCorrEta.Rda")
save(coeffs,file="coeffsCorrEpsCorrEta.Rda")
save(RMSEs,file="RMSEsCorrEpsCorrEta.Rda")
save(devs,file="devsCorrEpsCorrEta.Rda")
save(Ns,file="NsCorrEpsCorrEta.Rda")
save(fits,file="fitsCorrEpsCorrEta.Rda")
}
