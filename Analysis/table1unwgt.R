...
#--------------------------------------------
est1 <- feols(loginc    ~ educvar + advdegree + exper + I(exper^2) + I(exper^3) + racer + married | met2013, data=df)
est2 <- feols(birthmig  ~ educvar + advdegree + exper + I(exper^2) + I(exper^3) + racer + married | met2013, data=df)
est3 <- feols(dist      ~ educvar + advdegree + exper + I(exper^2) + I(exper^3) + racer + married | met2013, data=df %>% filter(birthState!=statefip))
est4 <- feols(relocc    ~ educvar + advdegree + exper + I(exper^2) + I(exper^3) + racer + married | met2013, data=df)
est5 <- feols(advdegree ~ educvar             + exper + I(exper^2) + I(exper^3) + racer + married | met2013, data=df)
...
for (f in levels(df$statefip)) {
    wage.est <- feols(loginc   ~ educvar + advdegree + exper + I(exper^2) + I(exper^3) + racer + married | met2013, data=df %>% filter(statefip==f))
    migr.est <- feols(birthmig ~ educvar + advdegree + exper + I(exper^2) + I(exper^3) + racer + married | met2013, data=df %>% filter(statefip==f))
    relo.est <- feols(relocc   ~ educvar + advdegree + exper + I(exper^2) + I(exper^3) + racer + married | met2013, data=df %>% filter(statefip==f))
    temp <- tibble(
                   state  = f,
                   w.ed   = 0, 
...
fname <- "../Paper/Tables/table1unwgt.tex"
...
cat("{\\footnotesize {\\raggedright Notes: Regression estimates at national level, controlling for demographics, advanced degree status, CBSA dummies, and a cubic in potential experience. Standard deviation of state-specific estimates reported below in parentheses. All variables except for log earnings and distance are expressed in percentage points and estimated from linear probability models. Sample taken from the 2010-2015 American Community Survey and is restricted to males ages 22-54 with a bachelor's degree or higher. Additional details on sample selection can be found in Table \\ref{tab:sampleselection}.}}",file=fname,sep="\n",append=TRUE)
